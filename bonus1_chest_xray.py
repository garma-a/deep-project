...
print("Downloading Chest X-Ray dataset directly from Kaggle...")
...
TRAIN_DIR = find_dir('train', dataset_path)
TEST_DIR = find_dir('test', dataset_path)
...
base_e2e = ResNet50(weights='imagenet', include_top=False, input_shape=(224, 224, 3))
...
for layer in base_e2e.layers[-50:]: layer.trainable = True
...
bars1 = ax.bar(x - width/2, svm_scores, width, label='Approach 1 (SVM)', color='steelblue')
bars2 = ax.bar(x + width/2, e2e_scores, width, label='Approach 2 (End-to-End)', color='darkorange')

ax.set_ylabel('Score')
ax.set_title('Bonus Comparative Analysis: SVM vs End-to-End ResNet50\n(Chest X-Ray Pneumonia)')
...
chart_path = f'{OUTPUT_DIR}/bonus_comparison_bar_chart.png'
